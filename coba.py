...
faceCascade = cv2.CascadeClassifier("C:\Program Files\Python37\Lib\site-packages\cv2\data\haarcascade_frontalface_alt2.xml")
...
while init:
	...
	faces = faceCascade.detectMultiScale(cv2image, scaleFactor=1.3, minNeighbors=5, minSize=(30,30), flags=cv2.CASCADE_SCALE_IMAGE)
	...
	for (x,y, w, h) in faces:
		...
		cv2.putText(vFrame,(str(emotions[predict[0]]) + str(": {:.2%}".format(float(_p[0][predict[0]])))), (x,y-10), cv2.FONT_HERSHEY_SIMPLEX,1, (255,255,255), 2)		
